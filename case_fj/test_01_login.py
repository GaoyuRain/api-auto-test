...
class TestLogin(unittest.TestCase):
    ...
    def test_01_get_verify_code(self):
        """获取验证码"""
        r = APIUtils.send_request(self.url_data.get('user_verify_code'))
        data = dict(r.json())
    ...
    def test_03_user_login_wx(self):
        r = APIUtils.send_request(self.url_data.get('user_login_wx'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_04_user_tourist_login(self):
        r = APIUtils.send_request(self.url_data.get('user_tourist_login'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_05_user_wx_login(self):
        r = APIUtils.send_request(self.url_data.get('user_wx_login'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_06_user_wx_bind(self):
        r = APIUtils.send_request(self.url_data.get('user_wx_bind'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_07_user_wx_mini_login(self):
        r = APIUtils.send_request(self.url_data.get('user_wx_mini_login'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_08_user_wx_mini_bind(self):
        r = APIUtils.send_request(self.url_data.get('user_wx_mini_bind'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_09_user_apple_login(self):
        r = APIUtils.send_request(self.url_data.get('user_apple_login'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_10_user_apple_bind(self):
        r = APIUtils.send_request(self.url_data.get('user_apple_bind'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_11_user_third_unbind(self):
        r = APIUtils.send_request(self.url_data.get('user_third_unbind'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_12_user_update(self):
        r = APIUtils.send_request(self.url_data.get('user_update'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_13_user_update_phone(self):
        r = APIUtils.send_request(self.url_data.get('user_update_phone'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_14_user_update_wx(self):
        r = APIUtils.send_request(self.url_data.get('user_update_wx'))
        data = dict(r.json())
        assert 200 == r.status_code

    def test_15_user_update_apple(self):
        r = APIUtils.send_request(self.url_data.get('user_update_apple'))
        data = dict(r.json())
