...
class TestMy(unittest.TestCase):
    ...
    def test_02_attention_list(self):
        r = APIUtils.send_request(self.url_data.get('user_attention_list'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None

    def test_03_user_fans(self):
        r = APIUtils.send_request(self.url_data.get('user_fans'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None
    ...
    def test_05_user_flow_albumlist(self):
        r = APIUtils.send_request(self.url_data.get('user_flow_albumlist'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None
    ...
    def test_07_user_notice(self):
        r = APIUtils.send_request(self.url_data.get('user_notice'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None

    def test_08_user_activity(self):
        r = APIUtils.send_request(self.url_data.get('user_activity'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None
