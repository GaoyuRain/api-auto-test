...
class TestAlbumInfo(unittest.TestCase):
    ...
    def test_05_buy_album(self):
        r = APIUtils.send_request(self.url_data.get('buy_album'))
        assert 200 == r.status_code
        message: dict = dict(r.json()).get('message')
        assert len(message) > 0

    def test_06_follow_album(self):
        r = APIUtils.send_request(self.url_data.get('follow_album'))
        assert 200 == r.status_code
        message: dict = dict(r.json()).get('message')
        assert len(message) > 0

    def test_07_album_history(self):
        r = APIUtils.send_request(self.url_data.get('album_history'))
        assert 200 == r.status_code
        message: dict = dict(r.json()).get('message')
        assert len(message) > 0
    ...
    def test_09_reward_buy(self):
        r = APIUtils.send_request(self.url_data.get('reward_buy'))
        assert 200 == r.status_code
        message: dict = dict(r.json()).get('message')
        assert len(message) > 0
