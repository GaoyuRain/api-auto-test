...
class TestCvInfo(unittest.TestCase):
    ...
    def test_01_cv_info(self):
        r = APIUtils.send_request(self.url_data.get('cv_info'))
    ...
    def test_02_album_cv_list(self):
        r = APIUtils.send_request(self.url_data.get('album_cv_list'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None

    def test_03_user_follow_cv(self):
        r = APIUtils.send_request(self.url_data.get('user_follow_cv'))
        assert 200 == r.status_code

    def test_04_cv_attention(self):
        r = APIUtils.send_request(self.url_data.get('cv_attention'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None

    def test_05_cv_fans(self):
        r = APIUtils.send_request(self.url_data.get('cv_fans'))
        assert 200 == r.status_code
        data: dict = dict(r.json()).get('data')
        assert data.get('list') is not None
